...
if ($user['role'] !== 'admin' && $existing['created_by'] != $user['id'] && $existing['assigned_to'] != $user['id']) {
    jsonResponse(['error' => 'Forbidden'], 403);
}

$result = $shipment->update($id, $input);
jsonResponse(['message' => 'Updated', 'shipment' => $result]);